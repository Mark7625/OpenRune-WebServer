package dev.openrune.cache.map.region
...
object HeightCalc {
    ...
    private val JAGEX_RADIAN = toRadians(ANGULAR_RATIO)
    ...
    init {
        for (i in 0 until JAGEX_CIRCULAR_ANGLE) {
            SIN[i] = (65536.0 * sin(i * JAGEX_RADIAN)).toInt()
            COS[i] = (65536.0 * cos(i * JAGEX_RADIAN)).toInt()
        }
    }

    fun calculate(x: Int, y: Int): Int {
        var n = interpolateNoise(x + 45365, y + 91923, 4) - 128 +
                ((interpolateNoise(x + 10294, y + 37821, 2) - 128) shr 1) +
    ...
    fun interpolateNoise(x: Int, y: Int, frequency: Int): Int {
        ...
        val v4 = smoothedNoise1(intX + 1, intY + 1)
    ...
    fun smoothedNoise1(x: Int, y: Int): Int {
        val corners = noise(x - 1, y - 1) + noise(x + 1, y - 1) +
                noise(x - 1, y + 1) + noise(x + 1, y + 1)
        val sides = noise(x - 1, y) + noise(x + 1, y) +
