...
class SpotAnimInfoHandler(private val config: ServerConfig) {
    
    private val manifest: Map<Int, SpotAnimManifestEntry> by lazy {
        @Suppress("UNCHECKED_CAST")
        loadSpotAnimData(config, dev.openrune.server.endpoints.sprites.DataType.WEB) as Map<Int, SpotAnimManifestEntry>
    }
    ...
    suspend fun handle(call: ApplicationCall) {
        try {
            ...
            logger.error("Error handling SpotAnim info request: ${e.message}", e)
            call.respond(
